#input  hourly aggregates by weekend for Q1 2018 Mumbai data file
my_mumbai_we_hr<-read.csv("/Users/Harsh/Desktop/Uber_Movement/mumbai-hexclusters-2018-1-OnlyWeekends-HourlyAggregate.csv", header = TRUE)
#Summary of data
str(my_mumbai_we_hr)
range(my_mumbai_we_hr$sourceid)
#load geojson file
library(jsonlite)
library(geosphere)
#load objects
...
my_mumbai_weekend_hourly_od<-read.csv("/Users/Harsh/Desktop/Uber_Movement/mumbai_weekends_hourly_OD.csv", header= TRUE)
#sumamry of merged data
str(my_mumbai_weekend_hourly_od)
my_mumbai_weekend_hourly_od[18222,]
library(spatstat)
...
my_route_d<-osrmRoute(src = my_mumbai_weekend_hourly_od[2575656,c("sourceid","lng_o","lat_o")],dst = my_mumbai_weekend_hourly_od[2575656,c("sourceid","lng_d","lat_d")], sp=TRUE)
install.packages("sp")
library(sp)
...
leaflet(my_route_d@lines[[1]]@Lines[[1]]@coords) %>% 
  ...
  addPolylines(data = route_coords, lng = route_coords$lon, lat = route_coords$lat, 
               label = paste(as.character(round(my_route_d$distance,2)), "KM,", as.character(round(my_mumbai_weekend_hourly_od$mean_travel_time[2565656]/60,2)),
                             "Minutes,", "HOD",as.character(my_mumbai_weekend_hourly_od$hod[2565656]), sep=" "),labelOptions = labelOptions(noHide = T, textsize = "12px"))

#calculate distance between test origin and destination- Test Case 2
my_route_d2<-osrmRoute(src = my_mumbai_weekend_hourly_od[178888,c("sourceid","lng_o","lat_o")],dst = my_mumbai_weekend_hourly_od[178888,c("sourceid","lng_d","lat_d")], sp=TRUE)
install.packages("sp")
library(sp)
...
leaflet(my_route_d2@lines[[1]]@Lines[[1]]@coords) %>% 
  ...
  addPolylines(data = route_coords2, lng = route_coords2$lon, lat = route_coords2$lat, 
               label = paste(as.character(round(my_route_d2$distance,2)), "KM,", as.character(round(my_mumbai_weekend_hourly_od$mean_travel_time[178888]/60,2)),
                             "Minutes,", "Hour of day=",as.character(my_mumbai_weekend_hourly_od$hod[178888]), sep=" "),labelOptions = labelOptions(noHide = T, textsize = "12px"))
